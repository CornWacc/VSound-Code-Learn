...
<script>
  export default {
    ...
    mounted() {
      this.$axios({
        url: "http://127.0.0.1:9055/base/code/codeInfoQuery?programId=" + this.$route.query.projectId + "&keyWord=" + "",
        method: "Get"
      }).then(res => {
    ...
    methods: {
      ...
      doSearch() {
        this.$axios({
          url: "http://127.0.0.1:9055/base/code/codeInfoQuery?programId=" + this.$route.query.projectId + "&codeName=" + this.codeSearch.codeName + "&codeType=" + this.codeSearch.codeType + "&codeLevel=" + this.codeSearch.codeLevel,
          method: "get"
        }).then(res => {
          if (res.data.status = "SUCCESS") {
            this.tableData = res.data.object.codeInfoList
      ...
      delCode(row) {
        this.$axios({
          url: "http://127.0.0.1:9055/base/code/delCode",
          data: {
            codeId: row.codeId,
          },
          method: "Post"
      ...
      sureCUCode() {
        ...
        if (this.dialog.dialogTital == "编辑源码" || this.codeUpdateForm.updateType == 'EXTRA') {
          this.$axios({
            url: "http://127.0.0.1:9055/base/code/codeUpdate",
          ...
          this.$axios({
            url: "http://127.0.0.1:9055/base/code/codeAdd",
            data: this.codeForm,
            method: "Post"
          }).then(res => {
            if (res.data.status == "SUCCESS") {
      ...
      sureCUDCodeMethod(row,type){
        ...
        this.$axios({
          url:"http://127.0.0.1:9055/base/code/codeMethodCUD",
          method:"Post",
          data:{
            type:type,
      ...
      sureCUDCodeParameter(row,type){
        ...
        this.$axios({
          url:"http://127.0.0.1:9055/base/code/codeParameterCUD",
          method:"Post",
          data:{
            type:type,
            codeId:this.drawerData.codeId,
      ...
      sureCUDCodeUrl(row,type){
        ...
        this.$axios({
          url:"http://127.0.0.1:9055/base/code/codeUrlCUD",
          method:"Post",
          data:{
            type:type,
            codeId:this.drawerData.codeId,
